...
This root `AGENTS.md` is repository-level guidance for 智码 GLM Code. Codex may also
...
智码 GLM Code is an Electron + React desktop application. Its core user-facing
...
`Cowork` is 智码 GLM Code's product/session layer. The name is historical: it
started as a Claude Code-like in-house coding assistant, but in the current
codebase it means the 智码 GLM Code layer that owns sessions, messages,
...
When changing OpenClaw-related behavior, first look for a 智码 GLM Code-side
...
智码 GLM Code when the behavior is product-specific or can be expressed cleanly at
the integration boundary.

Use version-scoped OpenClaw patches only when the required behavior is inside
OpenClaw and there is no clean 智码 GLM Code-side hook. Do not avoid a patch by
adding brittle or contorted 智码 GLM Code workarounds.
...
- `src/main/libs/openclawConfigSync.ts`: renders 智码 GLM Code state into
...
Important paths:
- `%APPDATA%/智码 GLM Code/openclaw/state/openclaw.json` on Windows: generated
  OpenClaw config.
- `%APPDATA%/智码 GLM Code/openclaw/state/workspace-main`: main agent workspace.
- `%APPDATA%/智码 GLM Code/openclaw/state/workspace-{agentId}`: non-main agent
  workspaces.
...
- `AGENTS.md`: OpenClaw workspace instructions with a 智码 GLM Code-managed section.
...
Main logs:
- Windows: `%APPDATA%/智码 GLM Code/logs/main-YYYY-MM-DD.log`
- macOS: `~/Library/Logs/智码 GLM Code/main-YYYY-MM-DD.log`
- Linux: `~/.config/智码 GLM Code/logs/main-YYYY-MM-DD.log`

Main log retention is 7 days. Max file size is 80 MB; overflow rotates to
`.old.log`.

OpenClaw gateway capture logs:
- Windows: `%APPDATA%/智码 GLM Code/openclaw/logs/gateway-YYYY-MM-DD.log`
- Retention is 3 days.
