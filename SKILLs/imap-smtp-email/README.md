...
1. **Configure accounts in 智码 GLM Code Settings > Email.**

智码 GLM Code writes `accounts.json` for multi-account setups. Existing `.env` files are still read as a legacy single-account fallback.
...
- `accounts.json` - Multi-account credentials managed by 智码 GLM Code Settings
