...
Modes:
  --pkg-base-url  one-pass build; the installer downloads <dir>/<keyfrom>/智码 GLM Code-<version>-x64.nsis.7z
...
if (usesPlaceholder) {
  ...
  console.log(`[WebBuild] next: upload release/智码 GLM Code-${version}-x64.nsis.7z, then run`);
