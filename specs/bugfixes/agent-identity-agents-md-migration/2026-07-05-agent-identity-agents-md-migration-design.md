...
用户在 智码 GLM Code 的 Agent 设置页修改“身份描述”后，当前 Agent workspace 中的
...
| `USER.md` | 关于用户的信息 |
| `AGENTS.md` | workspace 规则、技能策略、运行策略和 智码 GLM Code managed section |
...
```text
<!-- 智码 GLM Code managed: do not edit below this line -->
```
...
本修复只处理 智码 GLM Code 历史生成的高置信 legacy identity block：

- 位于 `AGENTS.md` 的 智码 GLM Code managed marker 之前；
- 位于文件顶部默认模板区域；
...
**And** `AGENTS.md` 其它用户内容和 智码 GLM Code managed section 保持不变
...
- 保留 marker 前用户内容；
- 写入 marker 后 智码 GLM Code managed section；
...
2. 同一次保存后，如果 `AGENTS.md` 顶部存在高置信 `## Identity（必须遵守）` 历史块，该块
   从 prompt 表面移除。
3. 清理后的 `AGENTS.md` 仍保留原来的默认模板内容、用户其它规则和 智码 GLM Code managed
   section。
