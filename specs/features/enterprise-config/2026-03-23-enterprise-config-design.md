# 智码 GLM Code ToB Enterprise Config Support

## Overview

Enable 智码 GLM Code to support enterprise (ToB) deployment by reading a pre-configured package on startup, syncing model/IM/skill/agent/MCP configurations into SQLite, and locking down the UI so users can work immediately without manual setup.
...
**Platform paths**:
- macOS: `~/Library/Application Support/智码 GLM Code/enterprise-config/`
- Windows: `%APPDATA%/智码 GLM Code/enterprise-config/`
...
**Goal:** Enable 智码 GLM Code to read a pre-configured enterprise package on startup, sync model/IM/skill/agent/MCP configs into SQLite, and lock down the UI — so enterprise users can run the app with zero manual setup.
...
```bash
mkdir -p ~/Library/Application\ Support/智码 GLM Code/enterprise-config/skills/test-skill
mkdir -p ~/Library/Application\ Support/智码 GLM Code/enterprise-config/agents
mkdir -p ~/Library/Application\ Support/智码 GLM Code/enterprise-config/mcp
```
...
```bash
rm -rf ~/Library/Application\ Support/智码 GLM Code/enterprise-config
```
