 ...
 *   setLanguage('en');
 *   const label = t('trayShowWindow'); // "Open 智码 GLM Code"
...
const translations: Record<LanguageType, Record<string, string>> = {
  zh: {
    // Tray menu
    trayShowWindow: '打开 智码 GLM Code',
    trayNewTask: '新建任务',
    trayViewCompletedTask: '查看完成的任务',
    trayCompletedTaskTooltip: '智码 GLM Code - {count} 个任务已完成',
    traySettings: '设置',
    trayOpenWebConsole: '打开 Web 控制台',
    trayQuit: '退出',
    ...
    coworkErrorLobsterAILoginExpired: '登录状态已过期，请重新登录后继续使用 智码 GLM Code 套餐模型。',
    ...
    coworkErrorQuotaExhausted:
      '积分额度已用完，请升级套餐后继续使用。[立即升级/充值](https://glmcoding.cn/pricing)',
    ...
    coworkGatewaySessionSyncTimeout: 'OpenClaw 引擎响应缓慢，消息尚未发送。请等待 1~2 分钟后重新发送；若频繁出现，请检查系统内存与磁盘占用，并将 智码 GLM Code 加入杀毒软件白名单。',
    ...
    dataMigrationBackupDialogTitle: '备份 智码 GLM Code 数据',
    dataMigrationRestoreDialogTitle: '导入 智码 GLM Code 数据备份',
    dataMigrationBackupArchiveFilter: '智码 GLM Code 备份包',
    dataMigrationAllFilesFilter: '所有文件',
    dataMigrationBackupBlockedByActiveWorkloads:
      '当前有正在运行的 Agent 或定时任务，请停止或等待任务完成后再备份。',
    dataMigrationRestoreProgressTitle: '正在导入 智码 GLM Code 数据',
  ...
  en: {
    // Tray menu
    trayShowWindow: 'Open 智码 GLM Code',
    trayNewTask: 'New Task',
    trayViewCompletedTask: 'View Completed Task',
    trayCompletedTaskTooltip: '智码 GLM Code - {count} completed task(s)',
    traySettings: 'Settings',
    trayOpenWebConsole: 'Open Web Console',
    trayQuit: 'Quit',
    ...
    coworkErrorLobsterAILoginExpired:
      'Your login session has expired. Sign in again to continue using 智码 GLM Code plan models.',
    ...
    coworkGatewaySessionSyncTimeout: 'The OpenClaw engine is responding slowly and your message has not been sent. Please wait a minute or two and resend. If this happens frequently, check system memory and disk usage, and add 智码 GLM Code to your antivirus allowlist.',
    ...
    // Data migration dialogs
    dataMigrationBackupDialogTitle: 'Back Up 智码 GLM Code Data',
    dataMigrationRestoreDialogTitle: 'Import 智码 GLM Code Data Backup',
    dataMigrationBackupArchiveFilter: '智码 GLM Code Backup',
    ...
    dataMigrationRestoreProgressTitle: 'Importing 智码 GLM Code data',
    dataMigrationRestoreProgressDesc:
      'Restoring the backup and validating data. 智码 GLM Code will restart automatically when finished.',
