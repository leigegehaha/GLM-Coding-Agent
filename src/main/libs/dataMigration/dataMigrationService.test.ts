...
test('createMigrationArchive excludes cache and log data and writes a manifest', () => {
  ...
  const userData = path.join(root, '智码 GLM Code');
  ...
  expect(entries).toContain('智码 GLM Code/.lobsterai-migration.json');
  expect(entries).toContain(`智码 GLM Code/${DB_FILENAME}`);
  expect(entries).toContain('智码 GLM Code/openclaw/state/openclaw.json');
  expect(entries).toContain('智码 GLM Code/SKILLs/demo/SKILL.md');
  ...
  const manifest = JSON.parse(
    fs.readFileSync(path.join(extractRoot, '智码 GLM Code', '.lobsterai-migration.json'), 'utf8'),
  ) as {
    format?: string;
    archiveRoot?: string;
  ...
  expect(manifest.archiveRoot).toBe('智码 GLM Code');
...
test('createMigrationArchive replaces the live sqlite database with the snapshot', () => {
  ...
  const extractRoot = extractArchive(archivePath);
  expect(readSqliteString(path.join(extractRoot, '智码 GLM Code', DB_FILENAME), 'SELECT value FROM kv WHERE key = ?', ['auth_tokens']))
    .toContain('snapshot-refresh');
  expect(fs.existsSync(path.join(extractRoot, '智码 GLM Code', `${DB_FILENAME}-wal`))).toBe(false);
});
...
test('inspectMigrationArchive rejects legacy Windows PowerShell archive root', () => {
  const root = makeTempDir();
  const legacyRoot = path.join(root, 'AppData', 'Roaming', '智码 GLM Code');
  ...
  expect(() => inspectMigrationArchiveSync(archivePath)).toThrow(/does not contain 智码 GLM Code user data/);
});

test('inspectMigrationArchive rejects archives without a migration manifest', () => {
  const root = makeTempDir();
  const sourceUserData = path.join(root, '智码 GLM Code');
...
test('inspectMigrationArchive rejects archives whose manifest does not match sqlite content', () => {
  const root = makeTempDir();
  const sourceUserData = path.join(root, 'source', '智码 GLM Code');
  ...
  const extractRoot = extractArchive(archivePath);
  const manifestPath = path.join(extractRoot, '智码 GLM Code', '.lobsterai-migration.json');
...
test('performPendingDataMigrationRestoreSync creates rollback and restores backup data', () => {
  const root = makeTempDir();
  const sourceUserData = path.join(root, 'source', '智码 GLM Code');
  const targetUserData = path.join(root, 'target', '智码 GLM Code');
...
test('performPendingDataMigrationRestoreSync replaces data in place and preserves runtime locks', () => {
  ...
  createMigrationArchiveSync({ userDataPath: sourceUserData, outputPath: archivePath });
  const extractRoot = extractArchive(archivePath);
  writeFile(path.join(extractRoot, '智码 GLM Code', 'backups', 'sqlite', 'snapshots', 'lobsterai-latest.sqlite'), 'legacy-source-backup');
  writeFile(path.join(extractRoot, '智码 GLM Code', 'sqlite-backups', 'lobsterai-latest.sqlite'), 'legacy-source-legacy-backup');
  writeFile(path.join(extractRoot, '智码 GLM Code', 'cowork', 'bin', 'node.cmd'), 'legacy-source-shim');
  writeFile(path.join(extractRoot, '智码 GLM Code', 'install-timing.log'), 'legacy-source-install-log');
  writeFile(path.join(extractRoot, '智码 GLM Code', 'skill-migrate.log'), 'legacy-source-skill-migrate-log');
  writeFile(path.join(extractRoot, '智码 GLM Code', 'Dictionaries', 'source.bdic'), 'legacy-source-dictionary');
  writeFile(path.join(extractRoot, '智码 GLM Code', 'Local Storage', 'leveldb', 'source.log'), 'legacy-source-local-storage');
  writeFile(path.join(extractRoot, '智码 GLM Code', 'Preferences'), 'legacy-source-preferences');
  writeFile(path.join(extractRoot, '智码 GLM Code', 'Session Storage', 'leveldb', 'source.log'), 'legacy-source-session-storage');
  writeFile(path.join(extractRoot, '智码 GLM Code', 'openclaw', 'logs', 'gateway-2026-06-10.log'), 'legacy-source-gateway-log');
  writeFile(path.join(extractRoot, '智码 GLM Code', 'openclaw', 'mcp-packages', 'demo', 'node_modules', 'native.node'), 'legacy-source-native');
  writeFile(path.join(extractRoot, '智码 GLM Code', 'openclaw', 'state', 'logs', 'commands.log'), 'legacy-source-commands-log');
  writeFile(path.join(extractRoot, '智码 GLM Code', 'runtimes', 'python', 'python.exe'), 'legacy-source-runtime');
  tar.create({
    sync: true,
    gzip: true,
    file: archivePath,
